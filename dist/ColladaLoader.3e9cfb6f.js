...
})({"b0JaX":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "5ed828163e9cfb6f";
"use strict";
...
},{}],"8mT2F":[function(require,module,exports,__globalThis) {
...
var _colladaParserJs = require("./collada/ColladaParser.js");
var _colladaComposerJs = require("./collada/ColladaComposer.js");
/**
 ...
 */ class ColladaLoader extends (0, _three.Loader) {
	 ...
	 */ parse(text, path) {
        ...
        const parser = new (0, _colladaParserJs.ColladaParser)();
        ...
        const composer = new (0, _colladaComposerJs.ColladaComposer)(library, collada, textureLoader, tgaLoader);
...
},{"three":"hJIVG","../loaders/TGALoader.js":"6aVKh","./collada/ColladaParser.js":"7G3je","./collada/ColladaComposer.js":"4qvee","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"6aVKh":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"three":"hJIVG","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"7G3je":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "ColladaParser", ()=>ColladaParser);
parcelHelpers.export(exports, "getElementsByTagName", ()=>getElementsByTagName);
parcelHelpers.export(exports, "parseStrings", ()=>parseStrings);
parcelHelpers.export(exports, "parseFloats", ()=>parseFloats);
parcelHelpers.export(exports, "parseInts", ()=>parseInts);
parcelHelpers.export(exports, "parseId", ()=>parseId);
var _three = require("three");
/**
 ...
 */ class ColladaParser {
    ...
    parse(text) {
        ...
        if (parserError !== undefined) {
            ...
            else errorText = this.parserErrorToText(parserError);
    ...
    parseAnimation(xml) {
        ...
        if (hasChildren === false) // since 'id' attributes can be optional, it's necessary to generate a UUID for unique assignment
        this.library.animations[xml.getAttribute('id') || (0, _three.MathUtils).generateUUID()] = data;
    }
        ...
        for(let i = 0, l = xml.childNodes.length; i < l; i++){
            ...
            switch(child.nodeName){
                case 'color':
                    ...
                    data.color = new (0, _three.Color)().fromArray(array);
                    (0, _three.ColorManagement).colorSpaceToWorking(data.color, (0, _three.SRGBColorSpace));
                    break;
                case 'falloff_angle':
    ...
    parseGeometryPrimitive(xml) {
        ...
        if (primitive.type === 'polygons') primitive.vcount = [
            primitive.p.length / primitive.stride
        ];
        return primitive;
    }
    // kinematics
    ...
    prepareNodes(xml) {
        ...
        for(let i = 0; i < elements.length; i++){
            ...
            if (element.hasAttribute('id') === false) element.setAttribute('id', this.generateId());
        }
    }
    parseNode(xml) {
        ...
        if (this.hasNode(data.id)) console.warn('THREE.ColladaLoader: There is already a node with ID %s. Exclude current node from further processing.', data.id);
        else this.library.nodes[data.id] = data;
        return data;
    }
    ...
    parseVisualScene(xml) {
        ...
        for(let i = 0; i < elements.length; i++)data.children.push(this.parseNode(elements[i]));
...
},{"three":"hJIVG","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"4qvee":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "ColladaComposer", ()=>ColladaComposer);
var _three = require("three");
var _colladaParserJs = require("./ColladaParser.js");
/**
 ...
 */ class ColladaComposer {
    constructor(library, collada, textureLoader, tgaLoader){
        ...
        this.tempColor = new (0, _three.Color)();
        this.animations = [];
        this.kinematics = {};
        // Reusable objects for animation
        this.position = new (0, _three.Vector3)();
        this.scale = new (0, _three.Vector3)();
        this.quaternion = new (0, _three.Quaternion)();
    ...
    compose() {
        ...
        const scene = this.parseScene((0, _colladaParserJs.getElementsByTagName)(this.collada, 'scene')[0]);
    ...
    buildAnimation(data) {
        ...
        for(const nodeId in aggregated){
            ...
            if (this.hasPivotTransforms(nodeData)) // Defer - nodes haven't been built yet
    ...
    collectDeferredPivotAnimation(nodeId, nodeChannels) {
        if (!this.deferredPivotAnimations[nodeId]) this.deferredPivotAnimations[nodeId] = {};
        const deferred = this.deferredPivotAnimations[nodeId];
        for(const sid in nodeChannels){
            if (!deferred[sid]) deferred[sid] = {};
            for(const member in nodeChannels[sid])deferred[sid][member] = nodeChannels[sid][member];
        }
    }
    hasPivotTransforms(nodeData) {
        ...
        for (const sid of pivotSids){
            if (nodeData.transforms[sid] !== undefined) return true;
        }
        return false;
    }
    getAnimation(id) {
    ...
    buildMatrixTracks(object3D, channelData, nodeData, tracks) {
        ...
        for(const member in channelData){
            ...
            for(let i = 0, il = times.length; i < il; i++){
                ...
                } else for(let j = 0; j < stride; j++)data[time][j] = values[valueOffset + j];
            }
        }
    ...
    buildTranslateTrack(object3D, channelData, transformInfo, tracks) {
        if (channelData.default && channelData.default.stride === 3) {
            ...
            const track = new (0, _three.VectorKeyframeTrack)(object3D.uuid + '.position', times, values);
    ...
    buildRotateTrack(object3D, sid, channelData, transformInfo, nodeData, tracks) {
        ...
        for (const transformSid of nodeData.transformOrder){
            ...
            if (transformType === 'rotate') {
                ...
                rotations.push({
                    sid: transformSid,
                    axis: new (0, _three.Vector3)(info.axis[0], info.axis[1], info.axis[2]),
                    defaultAngle: info.angle
                });
            }
        }
        const quaternion = new (0, _three.Quaternion)();
        const prevQuaternion = new (0, _three.Quaternion)();
        ...
        for(let i = 0; i < times.length; i++){
            ...
            for (const rotation of rotations){
                let angleDegrees;
                if (rotation.sid === sid) angleDegrees = this.getValueAtTime(angleData, time, rotation.defaultAngle);
                else angleDegrees = rotation.defaultAngle;
                const angleRadians = (0, _three.MathUtils).degToRad(angleDegrees);
        ...
        const track = new (0, _three.QuaternionKeyframeTrack)(object3D.uuid + '.quaternion', times, values);
        this.applyInterpolation(track, interpolationInfo);
        tracks.push(track);
    }
    buildScaleTrack(object3D, channelData, transformInfo, tracks) {
        if (channelData.default && channelData.default.stride === 3) {
            ...
            const times = Array.from(data.times);
            const values = Array.from(data.values);
            const track = new (0, _three.VectorKeyframeTrack)(object3D.uuid + '.scale', times, values);
            const interpolationInfo = this.getInterpolationInfo(channelData);
            this.applyInterpolation(track, interpolationInfo, channelData);
            tracks.push(track);
            return;
    ...
    getValueAtTime(componentData, time, defaultValue) {
        ...
        for(let i = 0; i < times.length; i++){
            ...
            if (times[i] > time) {
                ...
                if (interp === 'STEP') return v0;
                else if (interp === 'BEZIER' && componentData.inTangent && componentData.outTangent) return this.evaluateBezierComponent(componentData, i0, i1, t0, t1, time);
                else {
    ...
    getInterpolationInfo(channelData) {
        ...
        for (const comp of components){
            ...
            for(let i = 0; i < interpArray.length; i++){
                ...
                if (interpolationType === null) interpolationType = interp;
                else if (interp !== interpolationType) isUniform = false;
            }
        }
        return {
    ...
    applyInterpolation(track, interpolationInfo, channelData = null) {
        if (interpolationInfo.type === 'STEP' && interpolationInfo.uniform) track.setInterpolation((0, _three.InterpolateDiscrete));
        else if (interpolationInfo.type === 'BEZIER' && interpolationInfo.uniform && channelData) {
            const data = channelData.default;
            if (data && data.inTangent && data.outTangent) {
                track.setInterpolation((0, _three.InterpolateBezier));
                track.settings = {
    ...
    prepareAnimationData(data, defaultMatrix) {
        ...
        for(let i = 0; i < 16; i++)this.transformAnimationData(keyframes, i, defaultMatrix.elements[i]);
        return keyframes;
    }
    ...
    getImage(id) {
        ...
        if (data !== undefined) return this.getBuild(data, this.buildImage.bind(this));
    ...
    buildMaterial(data) {
        ...
        (0, _three.ColorManagement).colorSpaceToWorking(material.color, (0, _three.SRGBColorSpace));
        if (material.specular) (0, _three.ColorManagement).colorSpaceToWorking(material.specular, (0, _three.SRGBColorSpace));
        if (material.emissive) (0, _three.ColorManagement).colorSpaceToWorking(material.emissive, (0, _three.SRGBColorSpace));
        //
    ...
    getCamera(id) {
        ...
        if (data !== undefined) return this.getBuild(data, this.buildCamera.bind(this));
    ...
    buildLight(data) {
        ...
        if (data.parameters.falloffAngle) light.angle = (0, _three.MathUtils).degToRad(data.parameters.falloffAngle);
        return light;
    }
    getLight(id) {
        const data = this.library.lights[id];
        if (data !== undefined) return this.getBuild(data, this.buildLight.bind(this));
    ...
    buildGeometryData(primitive, source, offset, array, isColor = false) {
        ...
        function pushVector(i) {
            ...
            if (isColor) {
                ...
                tempColor.setRGB(array[startIndex + 0], array[startIndex + 1], array[startIndex + 2], (0, _three.SRGBColorSpace));
            ...
            for(let i = 0, l = vcount.length; i < l; i++){
                ...
                if (count === 4) {
                    ...
                    for(let k = 0; k < count; k++){
                        ...
                        vertices.push(new (0, _three.Vector3)(x, y, z));
                    }
                    // determine surface normal
                    const normal = new (0, _three.Vector3)();
                    const _triangle = new (0, _three.Triangle)();
                    ...
                    const vertices2D = [];
                    if (Math.abs(normal.x) > Math.abs(normal.y) && Math.abs(normal.x) > Math.abs(normal.z)) for(let k = 0; k < count; k++)vertices2D.push(new (0, _three.Vector2)(vertices[k].y, vertices[k].z));
                    else if (Math.abs(normal.y) > Math.abs(normal.z)) for(let k = 0; k < count; k++)vertices2D.push(new (0, _three.Vector2)(vertices[k].x, vertices[k].z));
                    else for(let k = 0; k < count; k++)vertices2D.push(new (0, _three.Vector2)(vertices[k].x, vertices[k].y));
                    const isClockWise = (0, _three.ShapeUtils).isClockWise(vertices2D);
                    if (isClockWise === true) vertices2D.reverse();
                    const faces = (0, _three.ShapeUtils).triangulateShape(vertices2D, []);
                    // build indices
    ...
    setupKinematics() {
        ...
        this.kinematics = {
            ...
            setJointValue: function(jointIndex, value) {
                ...
                } else console.warn('THREE.ColladaLoader: Joint ' + jointIndex + ' does not exist.');
            }
        };
    }
    buildTransformList(node) {
        ...
        for(let i = 0; i < xml.childNodes.length; i++){
            ...
            switch(child.nodeName){
                case 'matrix':
                    array = (0, _colladaParserJs.parseFloats)(child.textContent);
    ...
    buildNode(data) {
        ...
        // nodes
        for(let i = 0, l = nodes.length; i < l; i++)objects.push(this.getNode(nodes[i]));
        // instance cameras
        ...
        // instance nodes
        for(let i = 0, l = instanceNodes.length; i < l; i++)objects.push(this.getNode(instanceNodes[i]).clone());
        let object;
        ...
        if (type !== 'JOINT' && this.hasPivotTransforms(data)) return this.wrapWithTransformHierarchy(object, data);
        object.matrix.copy(matrix);
        object.matrix.decompose(object.position, object.quaternion, object.scale);
        return object;
    }
    wrapWithTransformHierarchy(contentObject, nodeData) {
        ...
        const rootNode = new (0, _three.Group)();
        ...
        for(let i = 0; i < transformOrder.length; i++){
            ...
            const transformNode = new (0, _three.Group)();
            ...
            switch(info.type){
                ...
                case 'rotate':
                    {
                        const axis = new (0, _three.Vector3)(info.axis[0], info.axis[1], info.axis[2]);
                        const angle = (0, _three.MathUtils).degToRad(info.angle);
                ...
                case 'matrix':
                    {
                        const matrix = new (0, _three.Matrix4)().fromArray(info.array).transpose();
    ...
    resolveMaterialBinding(keys, instanceMaterials) {
        ...
        for(let i = 0, l = keys.length; i < l; i++){
            ...
            } else materials.push(this.getMaterial(id));
        }
        return materials;
    }
    get fallbackMaterial() {
        if (this._fallbackMaterial === undefined) this._fallbackMaterial = new (0, _three.MeshBasicMaterial)({
    ...
    parseScene(xml) {
        const instance = (0, _colladaParserJs.getElementsByTagName)(xml, 'instance_visual_scene')[0];
    ...
    setupAnimations() {
        ...
        if (this.isEmpty(clips) === true) {
            if (this.isEmpty(this.library.animations) === false) {
                ...
                this.animations.push(new (0, _three.AnimationClip)('default', -1, tracks));
            }
        } else for(const id in clips)this.animations.push(this.getAnimationClip(id));
    }
    ...
    buildHierarchyTranslateTrack(transformNode, channelData, transformInfo, tracks) {
        if (channelData.default && channelData.default.stride === 3) {
            const data = channelData.default;
            const track = new (0, _three.VectorKeyframeTrack)(transformNode.uuid + '.position', Array.from(data.times), Array.from(data.values));
            const interpolationInfo = this.getInterpolationInfo(channelData);
            this.applyInterpolation(track, interpolationInfo, channelData);
            tracks.push(track);
            return;
        ...
        const track = new (0, _three.VectorKeyframeTrack)(transformNode.uuid + '.position', times, values);
        this.applyInterpolation(track, interpolationInfo);
        tracks.push(track);
    }
    buildHierarchyRotateTrack(transformNode, channelData, transformInfo, tracks) {
        ...
        const times = Array.from(angleData.times);
        if (times.length === 0) return;
        const axis = transformNode.userData.rotationAxis || new (0, _three.Vector3)(transformInfo.axis[0], transformInfo.axis[1], transformInfo.axis[2]);
        const quaternion = new (0, _three.Quaternion)();
        const prevQuaternion = new (0, _three.Quaternion)();
        const values = [];
        const interpolationInfo = this.getInterpolationInfo(channelData);
        ...
        const track = new (0, _three.QuaternionKeyframeTrack)(transformNode.uuid + '.quaternion', times, values);
        this.applyInterpolation(track, interpolationInfo);
        tracks.push(track);
    }
    buildHierarchyScaleTrack(transformNode, channelData, transformInfo, tracks) {
        if (channelData.default && channelData.default.stride === 3) {
            const data = channelData.default;
            const track = new (0, _three.VectorKeyframeTrack)(transformNode.uuid + '.scale', Array.from(data.times), Array.from(data.values));
            const interpolationInfo = this.getInterpolationInfo(channelData);
            this.applyInterpolation(track, interpolationInfo, channelData);
            tracks.push(track);
            return;
        ...
        const track = new (0, _three.VectorKeyframeTrack)(transformNode.uuid + '.scale', times, values);
        this.applyInterpolation(track, interpolationInfo);
        tracks.push(track);
    }
}

},{"three":"hJIVG","./ColladaParser.js":"7G3je","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["b0JaX"], null, "parcelRequire6840", {})

//# sourceMappingURL=ColladaLoader.3e9cfb6f.js.map
