...
})({"5u1KE":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "d8b4d4e876eebbad";
"use strict";
...
},{}],"amL9e":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "VRMLLoader", ()=>VRMLLoader);
var _three = require("three");
var _chevrotainModuleMinJs = require("../libs/chevrotain.module.min.js");
var _chevrotainModuleMinJsDefault = parcelHelpers.interopDefault(_chevrotainModuleMinJs);
/**
 ...
 */ class VRMLLoader extends (0, _three.Loader) {
    /**
	 ...
	 */ parse(data, path) {
        ...
        function createTokens() {
            const createToken = (0, _chevrotainModuleMinJsDefault.default).createToken;
            ...
            const Comment = createToken({
                name: 'Comment',
                pattern: /#.*/,
                group: (0, _chevrotainModuleMinJsDefault.default).Lexer.SKIPPED
            });
        ...
        function createVisitor(BaseVRMLVisitor) {
            ...
            class VRMLToASTVisitor extends BaseVRMLVisitor {
                ...
                vrml(ctx) {
                    ...
                    if (ctx.route) for(let i = 0, l = ctx.route.length; i < l; i++){
                ...
                node(ctx) {
                    ...
                    if (ctx.field) for(let i = 0, l = ctx.field.length; i < l; i++){
                ...
                field(ctx) {
                    ...
                    let result;
                    // SFValue
                    if (ctx.singleFieldValue) result = this.visit(ctx.singleFieldValue[0]);
                    // MFValue
        ...
        function parseTree(tree) {
            ...
            for(let i = 0, l = nodes.length; i < l; i++){
                ...
                if (object instanceof (0, _three.Object3D)) scene.add(object);
                if (node.name === 'WorldInfo') scene.userData.worldInfo = object;
            }
            return scene;
        }
        function buildNodeMap(node) {
            if (node.DEF) nodeMap[node.DEF] = node;
            const fields = node.fields;
            for(let i = 0, l = fields.length; i < l; i++){
                const field = fields[i];
                if (field.type === 'node') {
                    const fieldValues = field.values;
                    for(let j = 0, jl = fieldValues.length; j < jl; j++)buildNodeMap(fieldValues[j]);
                }
            }
        }
        function getNode(node) {
            // handle case where a node refers to a different one
            if (node.USE) return resolveUSE(node.USE);
        ...
        function buildNode(node) {
            ...
            if (build !== undefined && node.DEF !== undefined && build.hasOwnProperty('name') === true) build.name = node.DEF;
            return build;
        }
        function buildGroupingNode(node) {
            const object = new (0, _three.Group)();
            //
            const fields = node.fields;
            for(let i = 0, l = fields.length; i < l; i++){
                const field = fields[i];
                ...
                switch(fieldName){
                    ...
                    case 'parameter':
                        break;
                    case 'rotation':
                        const axis = new (0, _three.Vector3)(fieldValues[0], fieldValues[1], fieldValues[2]).normalize();
        ...
        function buildBackgroundNode(node) {
            ...
            // sky
            if (skyColor) {
                const skyGeometry = new (0, _three.SphereGeometry)(radius, 32, 16);
                const skyMaterial = new (0, _three.MeshBasicMaterial)({
                    fog: false,
                    side: (0, _three.BackSide),
                    depthWrite: false,
                    depthTest: false
                });
                ...
                } else skyMaterial.color.setRGB(skyColor[0], skyColor[1], skyColor[2], (0, _three.SRGBColorSpace));
                const sky = new (0, _three.Mesh)(skyGeometry, skyMaterial);
                group.add(sky);
            }
            // ground
            if (groundColor) {
                if (groundColor.length > 0) {
                    const groundGeometry = new (0, _three.SphereGeometry)(radius, 32, 16, 0, 2 * Math.PI, 0.5 * Math.PI, 1.5 * Math.PI);
                    const groundMaterial = new (0, _three.MeshBasicMaterial)({
                        fog: false,
                        side: (0, _three.BackSide),
                        vertexColors: true,
                        depthWrite: false,
                    ...
                    const ground = new (0, _three.Mesh)(groundGeometry, groundMaterial);
                    group.add(ground);
                }
            }
        ...
        function buildShapeNode(node) {
            ...
            let material = new (0, _three.MeshBasicMaterial)({
            ...
            for(let i = 0, l = fields.length; i < l; i++){
                ...
                switch(fieldName){
                    case 'appearance':
                        if (fieldValues[0] !== null) material = getNode(fieldValues[0]);
                        break;
                    case 'geometry':
                        if (fieldValues[0] !== null) geometry = getNode(fieldValues[0]);
                        break;
                    default:
                        console.warn('THREE.VRMLLoader: Unknown field:', fieldName);
                        break;
            ...
            if (geometry && geometry.attributes.position) {
                ...
                if (type === 'points') {
                    const pointsMaterial = new (0, _three.PointsMaterial)({
                        name: (0, _three.Loader).DEFAULT_MATERIAL_NAME,
                        color: 0xffffff,
                        opacity: material.opacity,
                        transparent: material.transparent
                    });
                    if (geometry.attributes.color !== undefined) pointsMaterial.vertexColors = true;
                    else // if the color field is NULL and there is a material defined for the appearance affecting this PointSet, then use the emissiveColor of the material to draw the points
                    if (material.isMeshPhongMaterial) pointsMaterial.color.copy(material.emissive);
                    object = new (0, _three.Points)(geometry, pointsMaterial);
                } else if (type === 'line') {
                    ...
                    if (geometry.attributes.color !== undefined) lineMaterial.vertexColors = true;
                    ...
                    if (material.isMeshPhongMaterial) lineMaterial.color.copy(material.emissive);
                    object = new (0, _three.LineSegments)(geometry, lineMaterial);
                } else {
                    // check "solid" hint (it's placed in the geometry but affects the material)
                    if (geometry._solid !== undefined) material.side = geometry._solid ? (0, _three.FrontSide) : (0, _three.DoubleSide);
                    // check for vertex colors
                    if (geometry.attributes.color !== undefined) material.vertexColors = true;
                    object = new (0, _three.Mesh)(geometry, material);
                }
            } else {
                object = new (0, _three.Object3D)();
        ...
        function buildAppearanceNode(node) {
            let material = new (0, _three.MeshPhongMaterial)();
            let transformData;
            const fields = node.fields;
            for(let i = 0, l = fields.length; i < l; i++){
                const field = fields[i];
                ...
                switch(fieldName){
                    case 'material':
                        ...
                        } else // if the material field is NULL or unspecified, lighting is off and the unlit object color is (0, 0, 0)
                    ...
                    case 'texture':
                        ...
                        if (textureNode !== null) {
                            if (textureNode.name === 'ImageTexture' || textureNode.name === 'PixelTexture') material.map = getNode(textureNode);
                        // MovieTexture not supported yet
                        }
                        break;
                    case 'textureTransform':
                        if (fieldValues[0] !== null) transformData = getNode(fieldValues[0]);
                        break;
                    default:
                        console.warn('THREE.VRMLLoader: Unknown field:', fieldName);
                        break;
            ...
            for(let i = 0, l = fields.length; i < l; i++){
                ...
                switch(fieldName){
                    case 'ambientIntensity':
                        break;
                    case 'diffuseColor':
                        materialData.diffuseColor = new (0, _three.Color)().setRGB(fieldValues[0], fieldValues[1], fieldValues[2], (0, _three.SRGBColorSpace));
                        break;
                    case 'emissiveColor':
                        materialData.emissiveColor = new (0, _three.Color)().setRGB(fieldValues[0], fieldValues[1], fieldValues[2], (0, _three.SRGBColorSpace));
                        break;
                    case 'shininess':
                        materialData.shininess = fieldValues[0];
                        break;
                    case 'specularColor':
                        materialData.specularColor = new (0, _three.Color)().setRGB(fieldValues[0], fieldValues[1], fieldValues[2], (0, _three.SRGBColorSpace));
                        break;
                    case 'transparency':
        ...
        function buildPixelTextureNode(node) {
            let texture;
            let wrapS = (0, _three.RepeatWrapping);
            let wrapT = (0, _three.RepeatWrapping);
            const fields = node.fields;
            for(let i = 0, l = fields.length; i < l; i++){
                const field = fields[i];
                const fieldName = field.name;
                ...
                switch(fieldName){
                    case 'image':
                        ...
                        texture = new (0, _three.DataTexture)(data, width, height);
                        texture.colorSpace = (0, _three.SRGBColorSpace);
                        texture.needsUpdate = true;
                        texture.__type = textureType; // needed for material modifications
                        break;
                    case 'repeatS':
                        if (fieldValues[0] === false) wrapS = (0, _three.ClampToEdgeWrapping);
                        break;
                    case 'repeatT':
                        if (fieldValues[0] === false) wrapT = (0, _three.ClampToEdgeWrapping);
                        break;
                    default:
                        console.warn('THREE.VRMLLoader: Unknown field:', fieldName);
                        break;
        ...
        function buildTextureTransformNode(node) {
            const transformData = {
                center: new (0, _three.Vector2)(),
                rotation: new (0, _three.Vector2)(),
                scale: new (0, _three.Vector2)(),
                translation: new (0, _three.Vector2)()
            };
            const fields = node.fields;
            for(let i = 0, l = fields.length; i < l; i++){
                const field = fields[i];
        ...
        function buildCamera(node, type) {
            const camera = type === 'PerspectiveCamera' ? new (0, _three.PerspectiveCamera)() : new (0, _three.OrthographicCamera)();
            ...
            for(let i = 0, l = fields.length; i < l; i++){
                ...
                switch(fieldName){
                    ...
                    case 'heightAngle':
                        ...
                        camera.fov = (0, _three.MathUtils).radToDeg(fieldValues[0]);
        ...
        function buildIndexedFaceSetNode(node) {
            ...
            for(let i = 0, l = fields.length; i < l; i++){
                ...
                switch(fieldName){
                    case 'color':
                        ...
                        if (colorNode !== null) color = getNode(colorNode);
                        break;
                    case 'coord':
                    ...
                    case 'normal':
                        ...
                        if (normalNode !== null) normal = getNode(normalNode);
                        break;
                    case 'texCoord':
                        const texCoordNode = fieldValues[0];
                        if (texCoordNode !== null) texCoord = getNode(texCoordNode);
                        break;
                    case 'ccw':
                        ccw = fieldValues[0];
                        break;
            ...
            if (coordIndex === undefined) {
                ...
                return new (0, _three.BufferGeometry)(); // handle VRML files with incomplete geometry definition
            }
            ...
            if (color) {
                if (colorPerVertex === true) {
                    ...
                    } else // if the colorIndex field is empty, then the coordIndex field is used to choose colors from the Color node
                    colorAttribute = toNonIndexedAttribute(triangulatedCoordIndex, new (0, _three.Float32BufferAttribute)(color, 3));
                } else if (colorIndex && colorIndex.length > 0) {
            ...
            if (normal) {
                if (normalPerVertex === true) {
                    ...
                    } else // if the normalIndex field is empty, then the coordIndex field is used to choose normals from the Normal node
                    normalAttribute = toNonIndexedAttribute(triangulatedCoordIndex, new (0, _three.Float32BufferAttribute)(normal, 3));
                } else // consider face normals
                if (normalIndex && normalIndex.length > 0) {
            ...
            } else // if the normal field is NULL, then the loader should automatically generate normals, using creaseAngle to determine if and how normals are smoothed across shared vertices
            ...
            if (texCoord) {
                ...
                } else // if the texCoordIndex field is empty, then the coordIndex array is used to choose texture coordinates from the TextureCoordinate node
                uvAttribute = toNonIndexedAttribute(triangulatedCoordIndex, new (0, _three.Float32BufferAttribute)(texCoord, 2));
            }
            const geometry = new (0, _three.BufferGeometry)();
            const positionAttribute = toNonIndexedAttribute(triangulatedCoordIndex, new (0, _three.Float32BufferAttribute)(coord, 3));
        ...
        function buildIndexedLineSetNode(node) {
            ...
            if (color) {
                if (colorPerVertex === true) {
                    ...
                    } else // if the colorIndex field is empty, then the colors are applied to each polyline of the IndexedLineSet in order.
                    colorAttribute = toNonIndexedAttribute(expandedLineIndex, new (0, _three.Float32BufferAttribute)(color, 3));
                } else if (colorIndex.length > 0) {
            ...
            //
            const geometry = new (0, _three.BufferGeometry)();
            const positionAttribute = toNonIndexedAttribute(expandedLineIndex, new (0, _three.Float32BufferAttribute)(coord, 3));
            geometry.setAttribute('position', positionAttribute);
            if (colorAttribute) geometry.setAttribute('color', colorAttribute);
            geometry._type = 'line';
            return geometry;
        ...
        function buildPointSetNode(node) {
            ...
            const geometry = new (0, _three.BufferGeometry)();
            geometry.setAttribute('position', new (0, _three.Float32BufferAttribute)(coord, 3));
            if (color) {
                const colorAttribute = new (0, _three.Float32BufferAttribute)(color, 3);
                convertColorsToLinearSRGB(colorAttribute);
        ...
        function buildBoxNode(node) {
            const size = new (0, _three.Vector3)(2, 2, 2);
            const fields = node.fields;
            for(let i = 0, l = fields.length; i < l; i++){
                const field = fields[i];
                const fieldName = field.name;
            ...
            const geometry = new (0, _three.BoxGeometry)(size.x, size.y, size.z);
            return geometry;
        }
        function buildConeNode(node) {
            ...
            const geometry = new (0, _three.ConeGeometry)(radius, height, 16, 1, openEnded);
            return geometry;
        }
        function buildCylinderNode(node) {
            ...
            const geometry = new (0, _three.CylinderGeometry)(radius, radius, height, 16, 1);
            return geometry;
        }
        function buildSphereNode(node) {
            ...
            const geometry = new (0, _three.SphereGeometry)(radius, 16, 16);
            return geometry;
        }
        function buildElevationGridNode(node) {
            ...
            const colors = [];
            const uvs = [];
            for(let i = 0; i < zDimension; i++)for(let j = 0; j < xDimension; j++){
                ...
                if (texCoord) {
                    ...
                    uvs.push(s, t);
                } else uvs.push(i / (xDimension - 1), j / (zDimension - 1));
            }
            // indices
            const indices = [];
            for(let i = 0; i < xDimension - 1; i++)for(let j = 0; j < zDimension - 1; j++){
            ...
            //
            const positionAttribute = toNonIndexedAttribute(indices, new (0, _three.Float32BufferAttribute)(vertices, 3));
            const uvAttribute = toNonIndexedAttribute(indices, new (0, _three.Float32BufferAttribute)(uvs, 2));
            let colorAttribute;
            let normalAttribute;
            // color attribute
            if (color) {
                if (colorPerVertex === false) {
                    ...
                    colorAttribute = new (0, _three.Float32BufferAttribute)(colors, 3);
                } else colorAttribute = toNonIndexedAttribute(indices, new (0, _three.Float32BufferAttribute)(colors, 3));
                convertColorsToLinearSRGB(colorAttribute);
            }
            // normal attribute
            if (normal) {
                if (normalPerVertex === false) {
                    ...
                    normalAttribute = new (0, _three.Float32BufferAttribute)(normals, 3);
                } else normalAttribute = toNonIndexedAttribute(indices, new (0, _three.Float32BufferAttribute)(normals, 3));
            } else normalAttribute = computeNormalAttribute(indices, vertices, creaseAngle);
            // build geometry
            const geometry = new (0, _three.BufferGeometry)();
            geometry.setAttribute('position', positionAttribute);
            geometry.setAttribute('normal', normalAttribute);
        ...
        function buildExtrusionNode(node) {
            ...
            // vertices
            const vertices = [];
            const spineVector = new (0, _three.Vector3)();
            const scaling = new (0, _three.Vector3)();
            const axis = new (0, _three.Vector3)();
            const vertex = new (0, _three.Vector3)();
            ...
            for(let i = 0; i < spineCount - 1; i++)for(let j = 0; j < crossSectionCount - 1; j++){
            ...
            if (beginCap === true || endCap === true) {
                const contour = [];
                for(let i = 0, l = crossSection.length; i < l; i += 2)contour.push(new (0, _three.Vector2)(crossSection[i], crossSection[i + 1]));
                const faces = (0, _three.ShapeUtils).triangulateShape(contour, []);
                const capIndices = [];
                ...
                // begin cap
                if (beginCap === true) {
                    for(let i = 0, l = capIndices.length; i < l; i += 3)if (ccw === true) indices.push(capIndices[i + 0], capIndices[i + 1], capIndices[i + 2]);
                    else indices.push(capIndices[i + 0], capIndices[i + 2], capIndices[i + 1]);
                }
                // end cap
                if (endCap === true) {
                    const indexOffset = crossSectionCount * (spineCount - 1); // references to the first vertex of the last cross section
                    for(let i = 0, l = capIndices.length; i < l; i += 3)if (ccw === true) indices.push(indexOffset + capIndices[i + 0], indexOffset + capIndices[i + 2], indexOffset + capIndices[i + 1]);
                    else indices.push(indexOffset + capIndices[i + 0], indexOffset + capIndices[i + 1], indexOffset + capIndices[i + 2]);
                }
            }
            const positionAttribute = toNonIndexedAttribute(indices, new (0, _three.Float32BufferAttribute)(vertices, 3));
        ...
        function parseFieldChildren(children, owner) {
            for(let i = 0, l = children.length; i < l; i++){
                ...
                if (object instanceof (0, _three.Object3D)) owner.add(object);
            }
        }
        ...
        const vA = new (0, _three.Vector3)();
        const vB = new (0, _three.Vector3)();
        const vC = new (0, _three.Vector3)();
        const uvA = new (0, _three.Vector2)();
        const uvB = new (0, _three.Vector2)();
        const uvC = new (0, _three.Vector2)();
        function computeAttributeFromIndexedData(coordIndex, index, data, itemSize) {
            ...
            return new (0, _three.Float32BufferAttribute)(array, itemSize);
        }
        function computeAttributeFromFaceData(index, faceData) {
            ...
            return new (0, _three.Float32BufferAttribute)(array, 3);
        }
        ...
        function toNonIndexedAttribute(indices, attribute) {
            ...
            return new (0, _three.Float32BufferAttribute)(array2, itemSize);
        }
        const ab = new (0, _three.Vector3)();
        const cb = new (0, _three.Vector3)();
        function computeNormalAttribute(index, coord, creaseAngle) {
            ...
            return new (0, _three.Float32BufferAttribute)(normals, 3);
        }
        function weightedNormal(normals, vector, creaseAngle) {
            const normal = new (0, _three.Vector3)();
            if (creaseAngle === 0) normal.copy(vector);
            else {
                for(let i = 0, l = normals.length; i < l; i++)if (normals[i].angleTo(vector) < creaseAngle) normal.add(normals[i]);
            }
            return normal.normalize();
        }
        function toColorArray(colors) {
            const array = [];
            for(let i = 0, l = colors.length; i < l; i += 3)array.push(new (0, _three.Color)(colors[i], colors[i + 1], colors[i + 2]));
            return array;
        }
        function convertColorsToLinearSRGB(attribute) {
            ...
            for(let i = 0; i < attribute.count; i++){
                ...
                (0, _three.ColorManagement).colorSpaceToWorking(color, (0, _three.SRGBColorSpace));
		 ...
		 */ function paintFaces(geometry, radius, angles, colors, topDown) {
            ...
            const colorAttribute = new (0, _three.BufferAttribute)(new Float32Array(geometry.attributes.position.count * 3), 3);
            ...
            for(let i = 0; i < indices.count; i++){
                ...
                for(let j = 1; j < thresholds.length; j++){
                    ...
                    if (topDown === true) // interpolation for sky color
                    {
                        if (position.y <= thresholdA.y && position.y > thresholdB.y) {
                            t = Math.abs(thresholdA.y - position.y) / Math.abs(thresholdA.y - thresholdB.y);
                            break;
                        }
                    } else // interpolation for ground color
                    if (position.y >= thresholdA.y && position.y < thresholdB.y) {
                        t = Math.abs(thresholdA.y - position.y) / Math.abs(thresholdA.y - thresholdB.y);
                        break;
                    }
        ...
        if (data.indexOf('#VRML V2.0') === -1) throw Error('THREE.VRMLLexer: Version of VRML asset not supported.');
...
class VRMLLexer {
    constructor(tokens){
        this.lexer = new (0, _chevrotainModuleMinJsDefault.default).Lexer(tokens);
    }
    lex(inputText) {
...
const CstParser = (0, _chevrotainModuleMinJsDefault.default).CstParser;
...
class Face {
    constructor(a, b, c){
        ...
        this.b = b;
        this.c = c;
        this.normal = new (0, _three.Vector3)();
    }
}
const TEXTURE_TYPE = {
    INTENSITY: 1,
...
},{"three":"hJIVG","../libs/chevrotain.module.min.js":"ihott","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"ihott":[function(require,module,exports,__globalThis) {
/*! chevrotain - v9.0.1 */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var R = (t, e)=>()=>(e || (e = {
            exports: {}
...
var k = R((exports, module)=>{
    ...
    function defaults() {
        ...
        return assignNoOverwrite.apply(void 0, __spreadArray([
            {}
        ], t));
    }
    ...
    exports.peek = peek;
    function timer(t) {
        var e = new Date().getTime(), r = t(), n = new Date().getTime(), i = n - e;
        return {
            time: i,
            value: r
        };
...
`
        ...
        r("Transform Patterns", function() {
            i = !1, a = h.map(n, function(g) {
                ...
                if (h.isRegExp(y)) {
                    var b = y.source;
                    return b.length === 1 && b !== "^" && b !== "$" && b !== "." && !y.ignoreCase ? b : b.length === 2 && b[0] === "\\" && !h.contains([
                        "d",
                        "D",
                        "s",
                        "S",
...
`),
    ...
    function ao(t, e, r) {
        var n = [], i = !1, a = h.compact(h.flatten(h.mapValues(t.modes, function(c) {
            return c;
...
var Xe = R((N)=>{
    ...
    function Cn(t) {
        V.forEach(t, function(e) {
            bn(e) || (N.tokenIdxToClass[N.tokenShortNameIdx] = e, e.tokenTypeIdx = N.tokenShortNameIdx++), Nr(e) && !V.isArray(e.CATEGORIES) && (e.CATEGORIES = [
                e.CATEGORIES
            ]), Nr(e) || (e.CATEGORIES = []), Fn(e) || (e.categoryMatches = []), wn(e) || (e.categoryMatchesMap = {});
        });
    }
...
` + f);
                ...
                w.forEach(n.lexerDefinitionWarning, function(p) {
                    ...
                    if (Ee.SUPPORT_STICKY ? (n.chopInput = w.IDENTITY, n.match = n.matchWithTest) : (n.updateLastIndex = w.NOOP, n.match = n.matchWithExec), o && (n.handleModes = w.NOOP), n.trackStartLines === !1 && (n.computeNewColumn = w.IDENTITY), n.trackEndLines === !1 && (n.updateTokenEndLineColumnLocation = w.NOOP), /full/i.test(n.config.positionTracking)) n.createTokenInstance = n.createFullToken;
...
` + i);
            ...
            for(var me; L < b;){
                ...
                if (c !== null) {
                    ...
                    for(var vr = L, an = Q, on = te, ct = !1; !ct && L < b;)for(A = y.charCodeAt(L), e = this.chopInput(e, 1), L++, a = 0; a < Ke; a++){
...
See: https://github.com/chevrotain/chevrotain/issues/564#issuecomment-349062346 for details.`;
        return Te.has(t, Un) && (r.CATEGORIES = t[Un]), Pr.augmentTokenTypes([
            r
        ]), Te.has(t, Gn) && (r.LABEL = t[Gn]), Te.has(t, Wn) && (r.GROUP = t[Wn]), Te.has(t, qn) && (r.POP_MODE = t[qn]), Te.has(t, Bn) && (r.PUSH_MODE = t[Bn]), Te.has(t, jn) && (r.LONGER_ALT = t[jn]), Te.has(t, Vn) && (r.LINE_BREAKS = t[Vn]), Te.has(t, Kn) && (r.START_CHARS_HINT = t[Kn]), r;
    }
    H.EOF = zn({
        name: "EOF",
        pattern: Eo.Lexer.NA
...
var Lr = R((Ne)=>{
    ...
    Ne.first = jt;
    function ai(t) {
        for(var e = [], r = t.definition, n = 0, i = r.length > n, a, o = !0; i && o;)a = r[n], o = Cr.isOptionalProd(a), e = e.concat(jt(a)), n = n + 1, i = r.length > n;
        return qt.uniq(e);
    }
...
var pi = R((he)=>{
    ...
    var Fo = Gt(), wo = Lr(), ui = k(), ci = Mr(), Do = ne(), fi = function(t) {
        ...
        return e.prototype.startWalking = function() {
            ...
            var a = li(r.referencedRule, r.idx) + this.topProd.name, o = n.concat(i), s = new Do.Alternative({
                definition: o
            }), c = wo.first(s);
            this.follows[a] = c;
        }, e;
 ...
 has ` + (t.alternation.definition.length + 1) + " alternatives.");
        ...
        buildLeftRecursionError: function(t) {
            var e = t.topLevelRule.name, r = Wo.map(t.leftRecursionPath, function(a) {
                ...
                e
            ]).join(" --> "), i = `Left Recursion found in grammar.
...
var Tt = R((j)=>{
    ...
    var zo = function(t) {
        ...
        return e.prototype.walkTerminal = function(r, n, i) {
            if (this.isAtEndOfPath && r.terminalType.name === this.nextTerminalName && r.idx === this.nextTerminalOccurrence && !this.found) {
                var a = n.concat(i), o = new O.Alternative({
                    definition: a
                });
    ...
    function Qo(t, e, r, n) {
        var i = "EXIT_NONE_TERMINAL", a = [
            i
        ], o = "EXIT_ALTERNATIVE", s = !1, c = e.length, f = c - n - 1, p = [], l = [];
        for(l.push({
            idx: -1,
            def: t,
            ruleStack: [],
...
var yt = R((C)=>{
    ...
    function Ri(t, e, r, n) {
        var i = new Oi(t, r);
        e.accept(i);
        var a = i.result, o = new os(e, t, r), s = o.startWalking(), c = new We.Alternative({
            definition: a
...
var jr = R((x)=>{
    ...
    function ls(t, e) {
        ...
        var n = r.allProductions, i = M.groupBy(n, Li), a = M.pick(i, function(s) {
            ...
            var c = M.first(s), f = e.buildDuplicateFoundError(t, s), p = Gr.getProductionDslName(c), l = {
                message: f,
    ...
    function Pi(t, e) {
        ...
        var n = r.alternations, i = M.reduce(n, function(a, o) {
            var s = M.dropRight(o.definition), c = M.map(s, function(f, p) {
                var l = cs.nextPossibleTokensAfter([
                    f
                ], [], null, 1);
                return M.isEmpty(l) ? {
            ...
            if (s.ignoreAmbiguities !== !0) {
                var c = o.idx, f = o.path, p = B.findAll(a, function(m) {
                    ...
                    var v = [
                        m.idx + 1,
                        c + 1
                    ], u = e.idx === 0 ? "" : e.idx, d = n.buildAlternationPrefixAmbiguityError({
                        topLevelRule: r,
                        alternation: e,
                        ambiguityIndices: v,
                        prefixPath: m.path
...
var et = R((ae)=>{
    ...
    var ys = k(), Gi = "MismatchedTokenException", Wi = "NoViableAltException", Bi = "EarlyExitException", qi = "NotAllInputParsedException", ji = [
        Gi,
        Wi,
        Bi,
        qi
    ...
    var Ps = function() {
        ...
        return t.prototype.initRecoverable = function(e) {
            ...
            for(var a = this, o = this.findReSyncTokenType(), s = this.exportLexerState(), c = [], f = !1, p = this.LA(1), l = this.LA(1), m = function() {
                var v = a.LA(0), u = a.errorMessageProvider.buildMismatchTokenMessage({
                    expected: i,
                    actual: p,
                    previous: v,
            ...
            for(var r = [], n = this.LA(1); this.tokenMatcher(n, e) === !1;)n = this.SKIP_TOKEN(), this.addToResyncTokens(n, r);
            ...
            var n = this.getHumanReadableRuleStack(), i = de.cloneArr(this.RULE_OCCURRENCE_STACK), a = {
                ruleStack: n,
                occurrenceStack: i,
                lastTok: e,
                lastTokOccurrence: r
...
var Hi = R((ke)=>{
    ...
    function Ms(t, e, r) {
        t.children[r] === void 0 ? t.children[r] = [
            e
        ] : t.children[r].push(e);
    }
    ke.addTerminalToCst = Ms;
    function bs(t, e, r) {
        t.children[e] === void 0 ? t.children[e] = [
            r
        ] : t.children[e].push(r);
    }
...
`).replace(/\n/g, `
    ...
    function Ji(t, e) {
        var r = [];
        for(var n in t)ve.isFunction(t[n]) && !ve.contains(Ws, n) && !ve.contains(e, n) && r.push({
...
Make sure that all grammar rule definitions are done before 'performSelfAnalysis' is called.`);
            ...
            if (a.call(this) === !0) for(var f = this.doSingleRepetition(o); a.call(this) === !0 && f === !0;)f = this.doSingleRepetition(o);
            ...
            this.attemptInRepetitionRecovery(this.atLeastOneInternal, [
                e,
                r
            ], a, le.AT_LEAST_ONE_IDX, e, rt.NextTerminalAfterAtLeastOneWalker);
            ...
            if (s.call(this) === !0) {
                ...
                }; this.tokenMatcher(this.LA(1), o) === !0;)this.CONSUME(o), a.call(this);
                ...
                ], c, le.AT_LEAST_ONE_SEP_IDX, e, rt.NextTerminalAfterAtLeastOneSepWalker);
            ...
            this.attemptInRepetitionRecovery(this.manyInternal, [
                e,
                r
            ], a, le.MANY_IDX, e, rt.NextTerminalAfterManyWalker, f);
            ...
            if (this.RULE_STACK.pop(), this.RULE_OCCURRENCE_STACK.pop(), this.cstFinallyStateUpdate(), this.RULE_STACK.length === 0 && this.isAtEndOfInput() === !1) {
                var e = this.LA(1), r = this.errorMessageProvider.buildNotAllInputParsedMessage({
                    firstRedundant: e,
...
var pa = R((tr)=>{
    ...
    var $r = et(), Zr = k(), fa = yt(), Qs = ce(), Js = function() {
        ...
        return t.prototype.initErrorHandler = function(e) {
            ...
            var c = this.LA(0), f = this.errorMessageProvider.buildNoViableAltMessage({
    ...
    var ha = Tt(), da = k(), eu = function() {
        ...
        return t.prototype.initContentAssist = function() {}, t.prototype.computeContentAssist = function(e, r) {
            ...
            return ha.nextPossibleTokensAfter([
                n
            ], r, this.tokenMatcher, this.maxLookahead);
...
var Ra = R((nr)=>{
    ...
    var oe = k(), Pe = ne(), tu = ft(), ma = Xe(), Ea = Ue(), ru = ce(), nu = Yt(), ir = {
        description: "This Object indicates the Parser is during Recording Phase"
    };
    Object.freeze(ir);
    var Ta = !0, ya = Math.pow(2, nu.BITS_FOR_OCCURRENCE_IDX) - 1, _a = Ea.createToken({
        name: "RECORDING_PHASE_TOKEN",
        pattern: tu.Lexer.NA
    });
    ma.augmentTokenTypes([
        _a
    ]);
 ...
 inside top level rule: <` + this.recordingProdStack[0].name + ">"));
            ...
            var a = oe.peek(this.recordingProdStack), o = e.ruleName, s = new Pe.NonTerminal({
                idx: r,
                nonTerminalName: o,
 ...
 inside top level rule: <` + this.recordingProdStack[0].name + ">"));
            ...
            var a = oe.peek(this.recordingProdStack), o = new Pe.Terminal({
                idx: r,
                terminalType: e
            });
    ...
    function Ot(t, e, r, n) {
        n === void 0 && (n = !1), ar(r);
        var i = oe.peek(this.recordingProdStack), a = oe.isFunction(e) ? e : e.DEF, o = new t({
            definition: [],
            idx: r
        });
    ...
    function au(t, e) {
        var r = this;
        ar(e);
        var n = oe.peek(this.recordingProdStack), i = oe.isArray(t) === !1, a = i === !1 ? t : t.DEF, o = new Pe.Alternation({
            definition: [],
            idx: e,
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["5u1KE"], null, "parcelRequire6840", {})

//# sourceMappingURL=VRMLLoader.76eebbad.js.map
